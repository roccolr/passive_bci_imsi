...
%       4 - bandpass butterworth filter in range [8,30] Hz
%       5 - bandpass butterworth filter in range [1,100] Hz, notch filter 50 Hz
%
%   OUTPUT:
...
%  (updated in April 2025 with varargin - 5)
    ...
    for j = 1:m
        ...
        if (artif == 2 || artif == 3)
            try
                ...
                data.data{1, run(j)}.trial = data.data{1, run(j)}.trial - data.data{1, run(j)}.trial(1)+1;
            catch ME
               warning(['artifact removal failed: ', ME.message]);
            end
        elseif artif == 4
            [b,a] = butter(5,[1 40]/(f_samp/2),'bandpass');
            data.data{1, run(j)}.X = filter(b,a,data.data{1, run(j)}.X);
        elseif artif == 5
            [b_n,a_n] = butter(2,[49,51]/(f_samp/2),'stop');
            data.data{1, run(j)}.X = filtfilt(b_n,a_n,data.data{1, run(j)}.X);

            [b,a] = butter(4, [1,100]/(f_samp/2),'bandpass');
            data.data{1, run(j)}.X = filtfilt(b,a,data.data{1, run(j)}.X);
        end
        ...
        for i = 1:n(m)
            ...
            for h = 1:k     
                ...
                %%%%%%%%%%%%% IF DA RIMUOVERE!!!
                %if t_stop-1 > size(data.data{1,run(j)}.X,1)
                %    signals(index,:) = [data.data{1,run(j)}.X(t_start:end, ch(h));zeros(t_stop-1-size(data.data{1,run(j)}.X,1),1)];
                %else

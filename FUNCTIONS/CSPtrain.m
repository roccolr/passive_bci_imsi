...
% The input classes can be 2, 3 or 4 in the current implementation.
%
%   INPUT:
...
%  last update:     2024/04/18
    ...
    elseif (sum(flag) == 3)
        ...
        if (nch < 2)
            ...
            % CSP matrices
            for i = 1:nb
                [W1(:,:,i), W2(:,:,i), W3(:,:,i), C1(:,:,i), C2(:,:,i), C3(:,:,i), n1, n2, n3] = CSP_3task(Xa(:,:,:,i),Xb(:,:,:,i),Xc(:,:,:,i), m);
            end
        end
    ...
    elseif (sum(flag) == 4)
        ...
        error('Current CSP implementation only allows 2, 3 or 4 classes!');
    end    
end
function [filtered_eeg, HD] = filterBankCOYLE(eeg, fsamp, varargin)
...
elseif (nargin < 2)
    ...
    % filter parameters
    order = 16;                 % * chosen arbitrarily era 10 %enza = 16 mic = 4
    attenuation = 60;           % * chosen arbitrarily %changed for Coyle's version = 60 %enza 60 mic = 20
    
    % filter bands
    fcutL = 8;                        % low  cut off frequency
    fcutH = 40;                       % high cut off frequency
    
    %fshif = 2;                        % shift for bands overlap
    %No overlap for Coyle's version
    fband = [4, 6, 10, 12];                        % band width
  
    % filtered signal init
    n_bands = length(fband); %8-12 Hz, 12-18 Hz, 18-28 Hz, 28-40 Hz
    f_shift = [0, 4, 10, 20]; %per shiftare le f_low nel ciclo while
    filtered_eeg = zeros([size(eeg) n_bands]); % dimensione di eeg e numero di bande i (720 x 500 x 4)
    ...
    while (i <= n_bands)
        ...
        f_low = fcutL + f_shift(i);
        f_high = f_low + fband(i);
        ...
        for j = 1:size(eeg,1)
            ...
            %qui è usato y = filtfilt(sos,g,x) con sos: second order section
            %coefficients matrix, g: scale factors vector, x: input signal

...
async def main():
    ...
    active_task = asyncio.create_task(passive_bci())
    passive_task = asyncio.create_task(active_bci())
    ...
    try:
        while True:
            ...
            #     hand_block = 1
            #     motivo_stato = "🔴 BLOCCATO (fatigue Eccessiva)"
            # elif concentration <= CONCENTRATION_THRESHOLD:
            #     hand_block = 1
            #     motivo_stato = "🔴 BLOCCATO (Poca concentration)"
            # else:
            #     hand_block = 0
            #     motivo_stato = "🟢 ATTIVO"
